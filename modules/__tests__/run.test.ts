import PPx from '../ppx.ts';
global.PPx = Object.create(PPx);
import type {NlTypes} from '@ppmdev/modules/types.ts';
import {echoExe} from '../echo.ts';
import {runPPb, runPPe} from '../run.ts';
...
describe('runPPb()', function () {
  ...
  it('all run options', () => {
    expect(runPPb({startwith: 'noactive', wait: 'wait', priority: 'normal', breakjob: true, log: true})).toBeTruthy();
  ...
  it('print description without color', () => {
    ...
    expect(spy).toHaveBeenCalledWith(`*run -noppb %0ppbw.exe -k ${echoExe} -e "test1"%:*wait 500,2`);

    // no coloring if fg option is not specified
    runPPb({desc: 'test2', bg: 'red'});
    expect(spy).toHaveBeenLastCalledWith(`*run -noppb %0ppbw.exe -k ${echoExe} -e "test2"%:*wait 500,2`);
  });
  it('print description with color', () => {
    expect(runPPb({desc: 'test', fg: 'red'})).toBeTruthy();
    expect(spy).toHaveBeenCalledWith(`*run -noppb %0ppbw.exe -k ${echoExe} -e "\\x1b[31mtest\\x1b[49;39m"%:*wait 500,2`);

    expect(runPPb({desc: 'test', fg: 'black', bg: 'red'})).toBeTruthy();
    expect(spy).toHaveBeenLastCalledWith(`*run -noppb %0ppbw.exe -k ${echoExe} -e "\\x1b[41;30mtest\\x1b[49;39m"%:*wait 500,2`);
  });
  it('full options', () => {
    expect(
      runPPb({
    ...
    expect(spy).toHaveBeenCalledWith(
      `*run -noppb -noactive -wait -normal -breakjob -log %0ppbw.exe -bootid:A -bootmax:Z -q -k *windowposition %%n,20,20%%:*windowsize %%n,200,200%%:*selectppx %n%%:${echoExe} -e "\\x1b[40;37mtest\\x1b[49;39m"%%:%%I"full options"%:*wait 500,2`
    );
  });
});
